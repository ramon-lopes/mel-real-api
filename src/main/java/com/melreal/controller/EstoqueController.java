...
public class EstoqueController {
    ...
    public ResponseEntity<EstoqueResponse> consultar() {
        ...
        return ResponseEntity.ok(new EstoqueResponse(disponivel, limite, disponivel == 0));
    }
    ...
    public ResponseEntity<ReservaResponse> reservar(@RequestBody ReservaRequest req) {
        if (req.quantidade() <= 0) {
            return ResponseEntity.badRequest().body(
                    new ReservaResponse(false, estoqueService.getDisponivel(), "Quantidade inválida.")
            );
        }
        boolean ok = estoqueService.reservar(req.quantidade());
        if (ok) {
            return ResponseEntity.ok(new ReservaResponse(true, estoqueService.getDisponivel(), "Reserva confirmada!"));
        } else {
            return ResponseEntity.ok(new ReservaResponse(false, estoqueService.getDisponivel(),
    ...
    public ResponseEntity<EstoqueResponse> liberar(@RequestBody ReservaRequest req) {
        if (req.quantidade() > 0) estoqueService.liberar(req.quantidade());
        int disponivel = estoqueService.getDisponivel();
        return ResponseEntity.ok(new EstoqueResponse(disponivel, estoqueService.getLimite(), disponivel == 0));
    }

    // ── POST /estoque/admin/login ─────────────────────────────────────────────
    @PostMapping("/admin/login")
    public ResponseEntity<?> login(@RequestBody Map<String, String> body) {
        if (!adminSenha.equals(body.get("senha"))) {
    ...
    public ResponseEntity<EstoqueResponse> atualizarLimite(
        ...
        if (!("Bearer " + adminToken).equals(auth)) return ResponseEntity.status(401).build();
